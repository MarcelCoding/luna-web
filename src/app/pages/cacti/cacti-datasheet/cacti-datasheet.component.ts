...
export class CactiDatasheetComponent implements OnInit, OnDestroy, ComponentCanDeactivate {
  ...
  constructor(
    ...
    this.searchedSpecies = combineLatest([ this.form.get('specie.name')!.valueChanges, this.form.get('genus.id')!.valueChanges ])
      .pipe(distinctUntilChanged(CactiDatasheetComponent.compare), map(([ specieName, genusId ]) => this.searchSpecies(specieName, genusId)));

    this.searchedForms = combineLatest([ this.form.get('form.name')!.valueChanges, this.form.get('specie.id')!.valueChanges ])
      .pipe(distinctUntilChanged(CactiDatasheetComponent.compare), map(([ formName, specieId ]) => this.searchForms(formName, specieId)));
  ...
  private loadCactus(cactus: Cactus): void {
    ...
    const age = !cactus.age ? undefined : CactiDatasheetComponent.format(Duration.fromISO(cactus.age));
